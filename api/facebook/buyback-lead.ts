...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') return res.status(200).json({ status: 'ok', endpoint: 'facebook-buyback-lead', method: 'GET', note: 'Send POST with lead data' });
  if (req.method !== 'POST') return res.status(405).json({ success: false, error: 'Method not allowed', receivedMethod: req.method });
