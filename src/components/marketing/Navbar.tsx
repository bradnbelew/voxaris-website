    ...
    <>
      ...
      <motion.header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          isScrolled ? 'bg-carbon-950/95 backdrop-blur-2xl border-b border-white/[0.06] shadow-[0_4px_20px_rgba(0,0,0,0.3)]' : 'bg-carbon-950/90 backdrop-blur-xl'
        }`}
        ...
        <nav className="container-wide">
          <div className="flex items-center justify-between h-16">
            <Link to="/" className="flex items-center shrink-0">
              <img
                src="/voxaris-logo-white-cropped.png"
                alt="Voxaris AI"
            ...
            <div className="hidden lg:flex items-center gap-1">
              {navLinks.map((link) => (
                <div key={link.label} className="relative">
                  {link.dropdown ? (
                    <button
                      className={`flex items-center gap-1.5 px-4 py-2 text-[13px] font-medium transition-all duration-200 ${openDropdown === link.label ? 'text-white' : 'text-white/50 hover:text-white/80'}`}
                    ...
                    <Link to={link.href} className={`px-4 py-2 text-[13px] font-medium transition-all duration-200 ${location.pathname === link.href ? 'text-gold-400' : 'text-white/50 hover:text-white/80'}`}>
                      {link.label}
                    </Link>
                  )}
                  {link.dropdown && (
                    <AnimatePresence>
                      {openDropdown === link.label && (
                        <motion.div
                          className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-64 p-2 bg-carbon-900 rounded-2xl border border-white/[0.08] shadow-[0_12px_40px_rgba(0,0,0,0.4)]"
                          initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.15 }}
                          onMouseEnter={() => setOpenDropdown(link.label)} onMouseLeave={() => setOpenDropdown(null)}
                        >
                          {link.dropdown.map((item) => (
                            <Link key={item.label} to={item.href} className="flex flex-col px-4 py-3 rounded-xl hover:bg-white/[0.05] transition-colors group">
            ...
            <button className="lg:hidden p-2 text-white/60" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} aria-label={isMobileMenuOpen ? "Close navigation menu" : "Open navigation menu"} aria-expanded={isMobileMenuOpen}>{isMobileMenuOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}</button>
          </div>
        </nav>
      </motion.header>
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div className="fixed inset-0 z-40 lg:hidden bg-carbon-950" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <div key={link.label}>
                    {link.dropdown ? (
                      <div className="space-y-3">
                        ...
                        <div className="grid gap-2 pl-1">{link.dropdown.map((item) => (<Link key={item.label} to={item.href} className="block py-2 text-lg text-white/60 hover:text-white transition-colors">{item.label}</Link>))}</div>
                      </div>
